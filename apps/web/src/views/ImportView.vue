<template>
  <AppPageShell>
    ...
    <AppPageContent class="space-y-6">
      <Card class="overflow-hidden border-border/60 bg-gradient-to-br from-card via-card to-muted/30">
        <div class="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between">
          <div class="space-y-2">
            <div class="flex items-center gap-2 flex-wrap">
              <h1 class="text-xl font-semibold tracking-tight">批量导入工作台</h1>
              <Badge v-if="hasActiveImports" variant="secondary">{{ importBatches.activeBatchCount }} 个任务进行中</Badge>
            </div>
            <p class="text-sm text-muted-foreground">
              支持 PDF、图片、ZIP 批量导入。开启 AI 初筛后，会在文本解析完成后追加 Agent 风格结论，并持续刷新状态。
            </p>
          </div>

          <div class="flex flex-col gap-3 rounded-xl border border-border/60 bg-background/80 p-4 md:min-w-[320px]">
            <div class="flex items-center justify-between gap-3">
              <div>
                <p class="text-sm font-medium">AI 初筛</p>
                <p class="text-xs text-muted-foreground">导入后自动生成通过 / 待定 / 淘汰结论</p>
              </div>
              <Switch :model-value="autoScreen" @update:model-value="onAutoScreenChange" />
            </div>
            <div class="flex items-center gap-2 text-xs text-muted-foreground">
              <span class="inline-flex h-2 w-2 rounded-full" :class="autoScreen ? 'bg-green-500' : 'bg-muted-foreground/50'" />
              {{ autoScreen ? '新建导入默认启用 AI 初筛' : '新建导入仅做解析，不自动初筛' }}
            </div>
          </div>
        </div>
      </Card>
      ...
      <div class="grid gap-3 md:grid-cols-3">
        <Card class="p-4 space-y-1.5">
          <p class="text-xs uppercase tracking-[0.18em] text-muted-foreground">批次总数</p>
          <p class="text-2xl font-semibold">{{ batches.length }}</p>
          <p class="text-xs text-muted-foreground">含历史导入与当前进行中的批次</p>
        </Card>
        <Card class="p-4 space-y-1.5">
          <p class="text-xs uppercase tracking-[0.18em] text-muted-foreground">运行中</p>
          <p class="text-2xl font-semibold">{{ importBatches.activeBatchCount.value }}</p>
          <p class="text-xs text-muted-foreground">会每 3 秒自动刷新，无需手动轮询</p>
        </Card>
        <Card class="p-4 space-y-1.5">
          <p class="text-xs uppercase tracking-[0.18em] text-muted-foreground">已完成文件</p>
          <p class="text-2xl font-semibold">{{ completedFiles }}/{{ totalFiles }}</p>
          <p class="text-xs text-muted-foreground">成功 {{ successFiles }} · 失败 {{ failedFiles }}</p>
        </Card>
      </div>
          ...
          <div class="space-y-4 p-5">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div class="flex items-center gap-2 self-start">
                ...
                >
                  <ChevronDown :class="['h-3.5 w-3.5 transition-transform', expandedBatches.has(b.id) ? 'rotate-180' : '']" />
                ...
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  class="h-8 gap-1.5 text-xs text-destructive hover:text-destructive"
            ...
            <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
              <div class="space-y-2">
                ...
                <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
                  ...
                  <span>成功 <span class="font-medium text-green-600">{{ b.successFiles }}</span></span>
                  <span>失败 <span class="font-medium text-destructive">{{ b.failedFiles }}</span></span>
                </div>
              </div>
              <div class="text-right">
                <p class="text-2xl font-semibold tabular-nums">{{ batchProgress(b.processedFiles, b.totalFiles) }}%</p>
                <p class="text-xs text-muted-foreground">{{ b.status === 'processing' ? '后台持续处理中' : '批次已收口' }}</p>
              </div>
            </div>
          </div>

          <div v-if="expandedBatches.has(b.id)" class="border-t bg-muted/20 px-5 py-4">
            <div v-if="loadingFiles[b.id]" class="space-y-2">
              <Skeleton class="h-16 w-full rounded-xl" v-for="i in 3" :key="i" />
            </div>
            <div v-else-if="!batchFiles[b.id]?.length" class="rounded-xl border border-dashed px-4 py-6 text-center text-sm text-muted-foreground">
              暂无文件明细
            </div>
                ...
                <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div class="min-w-0 space-y-2 flex-1">
                    <div class="flex items-center gap-2 flex-wrap">
                      ...
                      <span class="min-w-0 flex-1 truncate text-sm font-medium">{{ fileNameOf(f.originalPath) }}</span>
                      <span
                      ...
                      >
                        ...
                        <span class="ml-1 opacity-80">{{ screeningResult(f)?.screeningConclusion?.score }}分</span>
                      </span>
                      <Badge
                        v-else-if="f.stage === 'ai_screening'"
                        variant="secondary"
                        class="text-[11px]"
                      >
                        AI 初筛中
                    ...
                    <div class="flex items-center gap-2 flex-wrap text-xs text-muted-foreground">
                      ...
                      <span v-if="screeningResult(f)?.screeningSource">· {{ screeningSourceLabel(screeningResult(f)?.screeningSource) }}</span>
                    </div>

                    <p v-if="screeningResult(f)?.screeningConclusion?.summary" class="text-sm text-muted-foreground">
                      {{ screeningResult(f)?.screeningConclusion?.summary }}
                    </p>

                    <p v-if="screeningResult(f)?.screeningConclusion?.recommendedAction" class="text-sm text-foreground/80">
                      建议：{{ screeningResult(f)?.screeningConclusion?.recommendedAction }}
                    </p>

                    <p v-if="screeningResult(f)?.screeningError" class="text-xs text-amber-600">
                      AI 初筛不可用，已回退规则结论：{{ screeningResult(f)?.screeningError }}
                    </p>
...
const hasActiveImports = computed(() => importBatches.activeBatchCount.value > 0);
const totalFiles = computed(() => batches.value.reduce((sum, batch) => sum + batch.totalFiles, 0));
const completedFiles = computed(() => batches.value.reduce((sum, batch) => sum + batch.processedFiles, 0));
const successFiles = computed(() => batches.value.reduce((sum, batch) => sum + batch.successFiles, 0));
const failedFiles = computed(() => batches.value.reduce((sum, batch) => sum + batch.failedFiles, 0));
...
function canRerunBatchScreening(batch: typeof batches.value[number]) {
  return batch.status !== "processing" && batch.status !== "queued" && batch.status !== "cancelled";
}

/**
 * Structured data extraction from raw resume text.
 *
 * Uses regex patterns + heuristics to pull out candidate fields.
 * This is intentionally simple — replace with LLM parsing for production.
 */
...
// Chinese name heuristic: a line with 2-4 Chinese chars at the start
...
// Section headers commonly seen in Chinese resumes
...
export function parseResumeText(text: string): ParsedResume {
  ...
  // Email — take the first match
  const emails = text.match(EMAIL_PAT);
  const email = emails?.[0] ?? null;

  // Name — search first few lines for Chinese name pattern
  ...
  // Skills — lines or sections containing skill keywords
  const skills: string[] = [];
  ...
  // Work history — capture lines after "工作经历" / "项目经历" sections
  ...
  // Position — look for something like "应聘 岗位" or "申请 职位"
  let position: string | null = null;
  const posMatch = text.match(/(?:应聘|申请|求职)[：:\s]*([^\n,，]{2,20})/i);
  if (posMatch) position = posMatch[1].trim();
  else {
    // Fall back: first long line after name that's not a section header
    for (const line of lines) {

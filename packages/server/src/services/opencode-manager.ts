...
export class OpenCodeManager {
  ...
  private async trySpawn(port: number): Promise<void> {
    ...
    this.process = Bun.spawn(
      ["opencode", "web", "--hostname", this.host, "--port", String(port),
        "--session-dir", this.sessionBase, "--data-dir", this.dataDir],
  ...
  async ensureWorkspace(candidateId: string): Promise<{ sessionId: string; url: string; status: string }> {
    ...
    await db.insert(candidateWorkspaces).values({
      id: wsId, candidateId, opencodeSessionId,
      workspaceStatus: "active", lastAccessedAt: now, createdAt: now,
    });

    log(`[opencode] workspace created for ${candidateId}, session ${opencodeSessionId}`);
    return { sessionId: opencodeSessionId, url: `${this.baseUrl}/session/${opencodeSessionId}`, status: "active" };
  }
  ...
  private async buildCandidateContext(candidateId: string): Promise<string> {
    ...
    const interviewRows = await db.select().from(interviews).where(eq(interviews.candidateId, candidateId)).orderBy(desc(interviews.scheduledAt));
    ...
    const resumeTexts = resumeRows.map(r => r.extractedText ?? "").filter(Boolean).join("\n\n---\n\n");
    const interviewSummaries = interviewRows.map(i => {
      const eval_ = i.manualEvaluationJson ? JSON.parse(i.manualEvaluationJson as string) : null;
      return `第 ${i.round} 轮面试 | 状态: ${i.status}${eval_ ? ` | 评价: ${JSON.stringify(eval_)}` : ""}`;
    }).join("\n");
    const artifactSummaries = artifactRows.map(a => `【${a.type}】当前版本 v${a.currentVersion}`).join("\n");
    const tags: string[] = cand.tagsJson ? JSON.parse(cand.tagsJson as string) : [];

    return [
      `# 候选人信息`, `姓名: ${cand.name}`, `岗位: ${cand.position ?? "未知"}`,
      `工作年限: ${cand.yearsOfExperience ?? "未知"} 年`, `来源: ${cand.source}`,
      `技能标签: ${tags.join(", ") || "无"}`, ``,
      `## 简历内容`, resumeTexts || "(无简历文本)", ``,
      `## 面试记录`, interviewSummaries || "(无面试记录)", ``,
      `## AI 产物`, artifactSummaries || "(无 AI 产物)",
    ].join("\n");
  }
  ...
  private async createSession(context: string): Promise<string> {
    const res = await fetch(`${this.baseUrl}/api/session/create`, {
      ...
      body: JSON.stringify({ model: "default", systemPrompt: "你是面试管理工具的 AI 助手，正在辅助面试官对候选人进行面试准备工作。", context }),
